    ...
    def __init__(self,S,type2entity):
        ...
        for f,node in self.S.name2FNode.items():
            ...
            for entity in self.type2entity["ACF"]:
                ...
                self.S.problem.variables.add(names=["v_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["crt_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["fc_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["tau1_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["tau3_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["tau4_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
                self.S.problem.variables.add(names=["w_%s_%s"%(f,entity.name)],lb=[0],types=['C'])
    ...
    def setSpeedCompressionConstraint(self):
        ...
        for f,node in self.S.name2FNode.items():
            ...
            for entity in self.type2entity["ACF"]:
                ...
                self.S.problem.linear_constraints.add(lin_expr=[[variables1,[-1]+[1]*(len(variables1)-1)]],rhs=[0],senses=['E'])  #Eqn16
                self.S.problem.linear_constraints.add(lin_expr=[[["y"+suffix,"v"+suffix],[node.ScheCrsSpeed,-1]]],rhs=[0],senses=['L'])  #Eqn17
                self.S.problem.linear_constraints.add(lin_expr=[[["y"+suffix,"v"+suffix],[node.MaxCrsSpeed,-1]]],rhs=[0],senses=['G'])  #Eqn17
                self.S.problem.linear_constraints.add(lin_expr=[[["fc"+suffix,"tau1"+suffix,"v"+suffix,"tau3"+suffix,"tau4"+suffix],[-1/node.CrsStageDistance]+self.S.config["FUELCONSUMPPARA"]]],rhs=[0],senses=['L']) #Eqn 34
                ...
                q5=cplex.SparseTriple(ind1=["y"+suffix,"v"+suffix],ind2=["y"+suffix,"crt"+suffix],val=[node.CrsStageDistance,-1]) #Eqn39
                
                qs=[q1,q2,q3,q4,q5]
                for i in range(len(qs)):
                    self.S.problem.quadratic_constraints.add(quad_expr=qs[i],rhs=0,sense='L')
                    
            self.S.problem.linear_constraints.add(lin_expr=[[variables2,[1]*len(variables2)]],rhs=[node.ScheCrsTime],senses=['E']) #Eqn20
